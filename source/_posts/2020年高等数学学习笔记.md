...
​	
$$
\forall a \in R,\exist b \in R: b>a
$$
...
设E为非空实数集， $\exist M \in R$,

$\forall x \in E: x < M$ ，则称M是E的一个上界（下界？）。

E既有上界又有下界，成为有界的。

有界等价表达：$\exist M > 0, \forall x \in E: |x| \le M $

#### 上确界(最小上界)

E为非空实数集，$\exist \beta \in R$

(1) $\forall x \in E: x <= \beta（上界)$

(2)$\forall \delta > 0, \exist x_{\delta} \in E: x_{\delta} > \beta - \delta(上确界)$
